<template>
  <div class="custom-table-container" >
    ...
    <el-table :data="sortedData" style="width: 100%">
      ...
      <el-table-column label="MLST" >
        <template #default="scope">
          <span v-for="(mlst, idx) in scope.row.mlst.slice(0, 3)" :key="idx" class="mlst-tag">
            {{ mlst.mlst }} ({{ mlst.count }})
          </span>
        </template>
      </el-table-column>
    </el-table>
...
const sortedData = computed(() => {
  ...
  if (serotypeFilter.value) {
    data = data.filter(item => item.serotype.toLowerCase().includes(serotypeFilter.value.toLowerCase()));
  }
