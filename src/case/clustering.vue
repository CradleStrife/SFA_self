<template>
  <div style="display: flex; flex-direction: column; height: 400vh; overflow-y: scroll;">
    <!-- New Section -->
    <div
      style="
        flex: 1;
    ...
    >
      ...
      <div style="flex: 1; margin-right: 5px; overflow-y: auto;">
      ...
      <!-- Middle: Highlighted data filter (multiple clustering) -->
      <div style="flex: 1; margin-right: 10px; overflow-y: auto;">
        <h2 style="text-align: center; margin-bottom: 10px;">
        ...
        >
          ...
          <div style="margin-bottom: 20px;">
            ...
            <el-select
              ...
              style="width: 100%;"
            >
              <el-option label="local datastore" value="local"></el-option>
              <el-option label="global datastore" value="global"></el-option>
      ...
      <!-- Right Side: General data filter -->
      <div style="flex: 1; margin-left: 10px; overflow-y: auto;">
        <h2 style="text-align: center; margin-bottom: 10px;">
          General data filter
        </h2>
        <!-- Filters -->
    ...
    >
      ...
      <div style="flex: 4; position: relative;">
        ...
        <div style="display: flex; height: 100%; overflow-x: auto;">
          <svg id="chart" width="100%" height="100%" style="flex-grow: 1;"></svg>
        </div>
      </div>
      ...
      >
        ...
        <div
          ...
          "
        >
          <h3>Number of clusters selected for statistic: {{ numberOfNodes }}</h3>
          <el-slider
    ...
    >
      ...
      >
        ...
        <div
          style="width: 100%; display: flex; flex-direction: column; margin-right: 20px;"
        >
        ...
        >
          ...
          <div
            ...
            "
          >
            <h3 style="margin: 0; margin-right: 20px;">Attribute Statistic Bar Chart</h3>
            <el-button
              type="primary"
      ...
      >
        ...
        <div
          ...
          "
        >
        <h3 style="margin: 0; margin-right: 20px;">Attribute Statistic Table</h3>  
        <el-button
            type="primary"
            icon="el-icon-download"
            size="mini"
...
// 定义 AST 列
const astColumns = [
  'Amikacin Interpretation', 'Tobramycin Interpretation', 'Gentamicin Interpretation',
  'Amox/K Clav Interpretation', 'Amp/Sulbactam Interpretation', 'Pip/Tazo Interpretation',
  'Ampicillin Interpretation', 'Piperacillin Interpretation', 'Aztreonam Interpretation',
  'Cefepime Interpretation', 'Cefotaxime Interpretation', 'Cefoxitin Interpretation',
  'Ceftazidime Interpretation', 'Cefuroxime Interpretation', 'Chloramphenicol Interpretation',
  'Ciprofloxacin Interpretation', 'Levofloxacin Interpretation', 'Norfloxacin Interpretation',
  'Colistin Interpretation', 'Doripenem Interpretation', 'Ertapenem Interpretation',
  'Imipenem Interpretation', 'Meropenem Interpretation', 'Fosfomycin Interpretation',
  'Minocycline Interpretation', 'Tetracycline Interpretation', 'Nitrofurantoin Interpretation',
  'Trimeth/Sulfa Interpretation'
];

// 定义 SPI 列
const spiColumns = [
  'C63PI', 'CS54_island', 'HPI', 'SESS-LEE', 'SGI1', 'SGI2', 'SPI-1', 'SPI-2', 'SPI-3', 'SPI-4',
  'SPI-5', 'SPI-6', 'SPI-7', 'SPI-8', 'SPI-9', 'SPI-10', 'SPI-11', 'SPI-12', 'SPI-13', 'SPI-14'
];

// 定义 AMR 列
const amrColumns = [
  'Aac2-Ia_AGly', 'Aac3-IIa_AGly', 'Aac3-Iva_AGly', 'Aac3-VIa_AGly', 'Aac6-Aph2_AGly',
  'Aac6-Iaa_AGly', 'Aac6-If_AGly', 'AacAad_AGly', 'AadA_AGly', 'AadB_AGly', 'Ant6-Ia_AGly',
  'Aph3--Ia_AGly', 'Aph3-IIb_AGly', 'Aph3-III_AGly', 'Aph4-Ia_AGly', 'Aph7_AGly', 'AphA2_AGly',
  'Sat4A_AGly', 'Sat-2A_AGly', 'StrA_AGly', 'StrB_AGly', 'VanC1_Gly', 'VanR-C_Gly', 'VanS-C_Gly',
  'VanT_Gly', 'VanX-Yc_Gly', 'ACI-1_Bla', 'ACT-MIR_Bla', 'AmpC1_Ecoli_Bla', 'AmpC2_Ecoli_Bla',
  'AmpH_Bla', 'AMPH_Ecoli_Bla', 'CARB_Bla', 'CEPA-85_Bla', 'CfiA_Bla', 'CMY_Bla', 'CTX-M-1_Bla',
  'CTX-M-2_Bla', 'CTX-M-8_Bla', 'CTX-M-9_Bla', 'DHA_Bla', 'LAP_Bla', 'MrdA_Bla', 'OXA-1_Bla',
  'OXA-2_Bla', 'OXA-50_Bla', 'OXA-61_Bla', 'SHV-OKP-LEN_Bla', 'TEM-1D_Bla', 'Arr_Rif',
  'Cat-86_Phe', 'CatA1_Phe', 'CatA2_Phe', 'CatA3_Phe', 'CatB7_Phe', 'CatBx_Phe', 'CmlA_Phe',
  'FexA_Phe', 'FloR_Phe', 'DfrA1_Tmt', 'DfrA5_Tmt', 'DfrA7_Tmt', 'DfrA8_Tmt', 'DfrA23_Tmt',
  'DfrA27_Tmt', 'DfrA_Tmt', 'SulI_Sul', 'SulII_Sul', 'SulIII_Sul', 'Erm42_MLS', 'ErmB_MLS',
  'ErmF_MLS', 'LinB_MLS', 'LnuA_MLS', 'LnuC_MLS', 'LnuF_MLS', 'MphA_MLS', 'FosA2_Fcyn',
  'FosA_Fcyn', 'Mcr1_Colistin', 'Mcr3_Colistin', 'OqxA_Flq', 'OqxB_Flq', 'QepA_Flq',
  'Qnr-A_Flq', 'Qnr-S_Flq', 'QnrB_Flq', 'TetA_Tet', 'TetB_Tet', 'TetC_Tet', 'TetD_Tet',
  'TetG_Tet', 'TetL_Tet', 'TetM_Tet', 'TetO_Tet', 'TetQ_Tet', 'TetS_Tet', 'TetW_Tet', 'TetX_Tet'
];

// 定义 plasmid 列
const plasmidColumns = [
  'IncFIA(HI1)', 'IncHI1A', 'IncHI2A', 'IncI1-I(Alpha)', 'IncHI1B(R27)', 'IncB/O/K/Z',
  'IncHI2', 'IncR', 'IncN', 'IncQ1', 'IncX1', 'IncX4', 'IncFIB(S)', 'IncFII', 'IncFII(S)',
  'IncI(Gamma)', 'pKPC-CAV1321', 'ColpVC', 'Col156', 'Col(pHAD28)', 'Col440II'
];
...
const loadSalmonellaData = async () => {
  ...
  try {
    ...
    // 指定需要的字段
    const requiredData = ['results'].join(',');
...
onMounted(async () => {
  try {
    ...
    value2.value = [new Date(`${startYear}-01-01`), new Date(`${endYear}-12-31`)];
    await loadSalmonellaData();
  } catch (error) {
...
const selectedDatastoreMultiple = ref(null);
...
function filterClusteringData(clustering) {
  let dataFromAttributeFilters = [];
  let dataFromIdFilters = [];
  ...
  if (selectedIds.length > 0) {
    let data = salmonellaData.value.filter((item) => {
  ...
  // Combine data
  const unionData = [...dataFromAttributeFilters, ...dataFromIdFilters];
...
function createDendrogram(dataset, dataset_ids, source, n) {
  ...
  const clusters = agnes(truncated_data, {
    method: 'ward',
  ...
  function buildTree(node) {
    if (node.children && node.children.length > 0) {
      const leftChild = buildTree(node.children[0]);
      const rightChild = buildTree(node.children[1]);

      const clusterName = `cluster${cluster_count}`;
      cluster_count += 1;
  ...
  const rootNode = buildTree(clusters);
...
const onRunButtonClick = () => {
  ...
  clusterings.value.forEach((clustering, index) => {
    // 为每个聚类过滤其高亮数据
    const filteredHighlightedDataForClustering = filterClusteringData(clustering);
    ...
    if (uniqueCombinedData.length === 0) {
      ...
      ElMessage.warning(`聚类 ${index + 1}：过滤后的数据集为空，无法进行聚类。`);
    ...
    if (selectedAttributesForClustering.value.includes('plasmid')) {
      columnsToUseForClustering = columnsToUseForClustering.concat(plasmidColumns);
    }

    // 如果未选择任何属性，提示警告并返回
    if (columnsToUseForClustering.length === 0) {
      ElMessage.warning('请选择至少一个属性进行聚类。');
      clusteringResults.value.push(null); // 保持索引对齐
      return;
    }
    ...
    console.log('用于聚类的数据集：', dataset);
    if (dataset.length === 0 || dataset.some((arr) => arr.length === 0)) {
      console.error('数据集为空或包含空数组。');
      ElMessage.warning(`聚类 ${index + 1}：用于聚类的数据集为空或包含空数组。`);
      clusteringResults.value.push(null); // 保持索引对齐
      return;
    }
...
const attributesForStatistic = ['Source', 'Country', 'Brand', 'Serotype', 'MLST'];
...
const onTableFilterAttributeChange = () => {
  ...
  tableFilterValues.value = getTableFilterValues(selectedTableFilterAttribute.value);
};
...
const filteredDataForClusterTable = computed(() => {
  ...
  if (!selectedTableFilterAttribute.value || !selectedTableFilterValue.value) {
    ...
    data = dataForClusterTable.value.filter(
      (item) => item[selectedTableFilterAttribute.value] === selectedTableFilterValue.value
    );
  }
...
function drawAttributeBarChart(data) {
  ...
  const containerWidth = document.getElementById('attribute-bar-chart-container')
    .clientWidth;
  const containerHeight = document.getElementById('attribute-bar-chart-container')
    .clientHeight;
...
const downloadTableData = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Attribute Statistic Table');
