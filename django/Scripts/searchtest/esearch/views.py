...
from .es_call import esearch_local, esearch_assay, esearch_local_clustering
...
#             threads = []
#             for doc_id in doc_ids:
#                 thread = threading.Thread(target=process_single_delete, args=(doc_id, results))
#                 threads.append(thread)
#                 thread.start()
...
def process_isolate_country_source_assay():
    start_time = time.time()
    es = get_elasticsearch_client()
...
def process_serotypes_assay(start_date, end_date):
    start_time = time.time()
    es = get_elasticsearch_client()
    # Create the base query
    ...
    query = {
        ...
        "aggs": {
            "serotype": {
                "terms": {
                    "field": "serotype.keyword",  # Ensure this matches the field in your Elasticsearch index
                    "size": 348  # Increase size to get all serotypes and then filter
                }
            }
        }
    }
    ...
    index_a='assay_data'
    res = es.search(index=index_a, body=query, size=0)  # Set size to 0 to return only aggregations
    print(res)
    serotype_data = []
    # serotype = [c.strip() for c in serotype.split(',')]
    for serotype_bucket in res['aggregations']['serotype']['buckets']:
        serotype = serotype_bucket['key']  # Initialize serotype within the loop
        if serotype != "NA":  # Exclude "NA" serotype
            serotype_count = serotype_bucket['doc_count']
...
def process_mlst_assay(start_date, end_date):
    es = get_elasticsearch_client()
...
def process_source_assay(start_date, end_date):
    start_time = time.time()
    es = get_elasticsearch_client()
...
def geo_dis_assay(start_date, end_date, serotype, source, serotype_condition, source_condition):
    es = get_elasticsearch_client()
    ...
    if source:
        source_conditions = [{"match": {"isolate_source": {"query": s, "operator": "and"}}} for s in source]
        if source_condition == 'must':
            query["query"]["bool"]["must"].extend(source_conditions)
        else:
            query["query"]["bool"]["should"] = query["query"]["bool"].get("should", []) + source_conditions
    ...
    index_a = 'assay_data'
    res = es.search(index=index_a, body=query, size=348)
    country_counts = {}
    ...
    for hit in res['hits']['hits']:
        ...
        if country_name:
            # Ensure country_name is treated as a list if needed
            if isinstance(country_name, str):
                country_name = [country_name]

            # Try logging before unpacking
            

            # Iterate through the country name(s) and handle counts
...
def process_country_source_a(start_date, end_date):
    es = get_elasticsearch_client()
    ...
    query["aggs"] = {
        "serotypes": {
            "terms": {"field": "serotype.keyword", "size": 348},
            "aggs": {
                "date": {
                    "terms": {"field": "isolate_date", "size": 348},  # All pub_years associated with the serotype
                    "aggs": {
                        "top_countries": {
                            "terms": {"field": "countryname.keyword", "size": 348},  # Top countries per serotype per pub_year
                            "aggs": {
                                "top_sources": {
                                    "terms": {"field": "isolate_source.keyword", "size": 348}  # Top sources per serotype per pub_year
                                }
                            }
                        }
                    }
    ...
    res = es.search(index='assay_data', body=query, size=348)
    serotype_country_source_data = {}

    for serotype_bucket in res['aggregations']['serotypes']['buckets']:
        serotypes = serotype_bucket['key']

        # Handle serotype as array
        if isinstance(serotypes, list):
            serotypes = [s for s in serotypes if s != "NA"]  # Exclude 'NA' values
        else:
            serotypes = [serotypes] if serotypes != "NA" else []

        for serotype in serotypes:
            # Process each publication year bucket within the serotype
            for pub_year_bucket in serotype_bucket['date']['buckets']:
                pub_year_keys = pub_year_bucket['key_as_string'] if isinstance(pub_year_bucket['key_as_string'], list) else [pub_year_bucket['key_as_string']]
                ...
                for country_bucket in pub_year_bucket['top_countries']['buckets']:
                    ...
                    # Process each source bucket within the country bucket
                    for source_bucket in country_bucket['top_sources']['buckets']:
                        sources = source_bucket['key'] if isinstance(source_bucket['key'], list) else [source_bucket['key']]
                        source_count = source_bucket['doc_count']

                        # Only add sources to the corresponding country
                        for source in sources:
                            for pub_year in pub_year_keys:  # Iterate over each year if pub_year is an array
                                # Create a composite key to track combinations of serotype, country, source, and year
                                composite_key = f"{serotype}-{country}-{source}-{pub_year}"

                                if composite_key in serotype_country_source_data:
                                    # If the combination exists, aggregate the counts
                                    serotype_country_source_data[composite_key]['count'] += source_count
                                else:
                                    # Add the new combination with the current count
                                    serotype_country_source_data[composite_key] = {
                                        'serotype': serotype,
                                        'count': source_count,  # Use the aggregated count for the country and source
                                        'country': country,
                                        'source': source,
                                        'pub_years': pub_year
                                    }

    # Convert the dictionary back to a list for returning
    result_list = list(serotype_country_source_data.values())

    # Filter the result to only include entries with "Salmonella Typhimurium" and source "Humans, Food, Swine"
    # filtered_result_list = [
    #     item for item in result_list
    #     if item['serotype'] == 'Salmonella enterica subspecies enterica serovar Gallinarum biovar Pullorum' and item['source'] == 'Salmonella enterica subspecies enterica serovar Gallinarum biovar Pullorum strains'
    # ]

    # Count the total number of filtered entries
    # total_filtered_count = len(filtered_result_list)

    # print(f"Total number of filtered entries: {total_filtered_count}")
    # print(f"Filtered serotype_country_source_data: ", filtered_result_list)
    return result_list

def process_mlst_table_a(start_date, end_date):
    es = get_elasticsearch_client()
    ...
    query["aggs"] = {
        "mlst": {
            "terms": {"field": "mlst.keyword", "size": 348},
            "aggs": {
                "date": {
                    "terms": {"field": "isolate_date", "size": 348},  # All pub_years associated with the serotype
                    "aggs": {
    ...
    res = es.search(index='assay_data', body=query, size=7000)
    mlst_table_data = {}

    for mlst_bucket in res['aggregations']['mlst']['buckets']:
        mlst = mlst_bucket['key']

        # Handle mlst as array
        if isinstance(mlst, list):
            mlst = [m for m in mlst if m != "NA"]  # Exclude 'NA' values
        else:
            mlst = [mlst] if mlst != "NA" else []

        for mlst_item in mlst:
            # Process each publication year bucket within the mlst
            for pub_year_bucket in mlst_bucket['date']['buckets']:
                pub_year_keys = pub_year_bucket['key_as_string'] if isinstance(pub_year_bucket['key_as_string'], list) else [pub_year_bucket['key_as_string']]
                ...
                for country_bucket in pub_year_bucket['top_countries']['buckets']:
                    ...
                    for source_bucket in country_bucket['top_sources']['buckets']:
                        ...
                        for source in sources:
                            for pub_year in pub_year_keys:  # Iterate over each year if pub_year is an array
                                # Create a composite key to track combinations of mlst, country, source, and year
                                composite_key = f"{mlst_item}-{country}-{source}-{pub_year}"

                                if composite_key in mlst_table_data:
                                    # If the combination exists, aggregate the counts
                                    mlst_table_data[composite_key]['count'] += source_count
                                else:
                                    # Add the new combination with the current count
                                    mlst_table_data[composite_key] = {
                                        'mlst': mlst_item,
                                        'count': source_count,  # Use the aggregated count for the country and source
                                        'country': country,
                                        'source': source,
    ...
    # Convert the dictionary back to a list for returning
    result_list = list(mlst_table_data.values())
    
    # You can apply filtering here if needed for specific MLST or source types
    filtered_result_list = [
        item for item in result_list
        if item['mlst'] == 'ST516' and item['source'] == 'Humans, Food, Swine'
    ]
    total_filtered_count = len(filtered_result_list)
    print("Filtered Results:", filtered_result_list)
    # print("Results:", result_list)
    return result_list



def process_ID_MLST_a(start_date, end_date):
    es = get_elasticsearch_client()
   
    # Create the query using the script to handle multiple dates in the array
    start_date='1700'
    end_date='2024'
    query = {
    "query": {
    ...
    for hit in res['hits']['hits']:
        record = hit['_source']
        pmcid = record.get("mlst")  # Get the pmcid value
    
    # Check if pmcid exists and is not None
        if pmcid:
