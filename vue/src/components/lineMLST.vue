        ...
        <div class="legend-container">
          <el-scrollbar style="height: 150px;">
            <div v-for="group in filteredGroupsMLSTAll" :key="group" class="legend-item">
              <span :style="{ color: colorMapMLSTAll[group] }">■</span> {{ group }}: {{ groupCountsMLSTAll[group] }}
            </div>
          </el-scrollbar>
        </div>
      </el-col>
...
const filteredGroupsMLST = computed(() => {
  const counts: { [key: string]: number } = {};
  processedDataMLST.value.forEach(item => {
  ...
  const filtered = Object.keys(counts).filter(group => {
    const [mlst, country, source] = group.split('-');
    return (
      (!selectedMLST.value || mlst.includes(selectedMLST.value)) &&
      (!selectedCountry.value || country.includes(selectedCountry.value)) &&
      (!selectedSource.value || source.includes(selectedSource.value))
    );
  });
...
watch(
  [combinations, selectedPeriodMLST, chartDataMLST], 
...
.legend-container {
  ...
  flex-wrap: wrap;
  justify-content: center;
  width: 100%;;/* Center the content within the row */
}
.legend-item {
  margin-right: 10px;
}
